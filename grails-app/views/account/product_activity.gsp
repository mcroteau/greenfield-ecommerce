...
<html>
	<head>
		...
		<title>Greenfield : Product Activity</title>
	...
	<body>
		...
		<div id="list-account" class="content scaffold-list" role="main">
			...
			<g:link controller="account" action="account_activity" id="${accountInstance.id}" class="btn btn-default pull-right" name="Back to Account Activity">Back to Activity</g:link>
			...
			<p class="information secondary" style="margin-bottom:0px;">${accountInstance.username}'s overall product viewing activity</p>
			...
			<g:if test="${productViewStats}">
				...
				<div id="activity-stats-wrapper" class="pull-left">
					<table class="table" style="width:400px">
						...
						<g:each in="${productViewLogs}" var="productViewLog">
							<tr>
								<td>${productViewLog.product.name}</td>
								<td><g:formatDate format="hh:mm z - dd MMM yyyy " date="${productViewLog.dateCreated}"/></td>
							</tr>
						</g:each>
					</table>
				</div>
